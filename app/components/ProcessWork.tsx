...
const ProcessWork = () => {
  ...
  return (
    <section className="py-12 sm:py-16 lg:py-20 px-4 bg-gradient-to-br from-gray-50 to-blue-50 flex flex-col items-center justify-center">
      <div className="w-full max-w-7xl">
        {/* Header */}
        <div className="flex flex-col items-center justify-center text-center mb-12 sm:mb-16">
          <div className="inline-flex items-center justify-center px-3 py-2 sm:px-4 sm:py-2 bg-blue-100 text-blue-800 rounded-full text-xs sm:text-sm font-semibold mb-3 sm:mb-4">
            🔄 Proses Kerja
          </div>
        ...
        <div className="flex justify-center items-center mb-12 sm:mb-16">
          ...
          <div className="block lg:hidden space-y-6 w-full max-w-2xl mx-auto">
            {steps.map((step, index) => (
              <div key={step.id} className="relative flex justify-center items-center">
                <div className="w-full max-w-lg mx-auto justify-center items-center flexr ">
                  {/* Connector Line */}
                          ...
                          <div className="mt-4 space-y-2">
                            {step.details.map((detail, idx) => (
                              <div key={idx} className="flex items-start justify-center gap-2">
                                ...
                                <span className="text-sm text-gray-700 text-left">{detail}</span>
                              </div>
                            ))}
                          </div>
                        )}
          ...
          <div className="hidden lg:flex lg:flex-col lg:items-center lg:justify-center">
                ...
                <div className="flex items-center justify-between w-full max-w-4xl">
                  {steps.map((step, index) => (
                    <div key={step.id} className="flex items-center flex-1">
                      <div 
                        className={`relative cursor-pointer transition-all duration-300 mx-auto ${
                      ...
                      {/* Connector Line */}
                      {index < steps.length - 1 && (
                        <div className="flex-1 h-1 mx-8 bg-gradient-to-r from-blue-200 to-blue-100 rounded-full"></div>
                      )}
                    </div>
                  ))}
                </div>
            ...
            <div className="flex justify-center">
              <div className="bg-white rounded-3xl p-8 shadow-xl border border-gray-100 w-full max-w-4xl">
                {steps.map((step) => (
                  activeStep === step.id && (
                    <div key={step.id} className="text-center">
                      <div className="text-6xl mb-6">{step.icon}</div>
                      <h3 className="text-3xl font-bold text-gray-900 mb-4">{step.title}</h3>
                      <p className="text-xl text-gray-600 mb-8 max-w-2xl mx-auto">{step.description}</p>
                      
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-3xl mx-auto">
                        {step.details.map((detail, idx) => (
                          <div key={idx} className="flex items-center justify-center gap-3 p-4 bg-blue-50 rounded-xl">
        ...
        {/* CTA Section */}
        <div className="flex justify-center">
          <div className="bg-gradient-to-r from-blue-600 to-blue-700 rounded-2xl sm:rounded-3xl p-6 sm:p-8 lg:p-12 text-white text-center w-full max-w-5xl">
            <div className="flex flex-col items-center justify-center">
              <h3 className="text-2xl sm:text-3xl lg:text-4xl font-bold mb-3 sm:mb-4">
                Siap bikin brand Anda berbicara?
              </h3>
              <p className="text-base sm:text-lg lg:text-xl text-blue-100 mb-6 sm:mb-8 max-w-2xl text-center">
              ...
              <div className="grid grid-cols-3 gap-4 sm:gap-8 max-w-2xl w-full">
                <div className="bg-white/10 backdrop-blur-sm rounded-xl sm:rounded-2xl p-3 sm:p-4 text-center">
                  <div className="text-xl sm:text-2xl font-bold text-white mb-1">24h</div>
                  <div className="text-gray-300 text-xs sm:text-sm">Express Service</div>
                </div>
                <div className="bg-white/10 backdrop-blur-sm rounded-xl sm:rounded-2xl p-3 sm:p-4 text-center">
                  <div className="text-xl sm:text-2xl font-bold text-white mb-1">5★</div>
                  <div className="text-gray-300 text-xs sm:text-sm">Rating Klien</div>
                </div>
                <div className="bg-white/10 backdrop-blur-sm rounded-xl sm:rounded-2xl p-3 sm:p-4 text-center">
                  <div className="text-xl sm:text-2xl font-bold text-white mb-1">7+</div>
                  <div className="text-gray-300 text-xs sm:text-sm">Tahun Pengalaman</div>
                </div>
              </div>
            </div>
          </div>
